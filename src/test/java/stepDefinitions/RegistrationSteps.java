...
public class RegistrationSteps {
	...
	public void the_user_clicks_button_with_entering_only_username_field(String string,
	...
	public void the_user_clicks_button_with_entering_only_and(String string, String string2, String string3,
			io.cucumber.datatable.DataTable dataTable) {
	...
	public void the_user_enters_a_valid_and_with_less_than_characters(String string, String string2, Integer int1,
			io.cucumber.datatable.DataTable dataTable) {
		// Write code here that turns the phrase above into concrete actions
		registerPage.validname(dataTable);
		registerPage.validConfPw(dataTable);
	...
	@When("The user submits registration details from {string} for row {int}")
	public void the_user_submits_registration_details_from_for_row(String sheetName, int rowIndex) throws IOException {
		outputAndValidation = registerPage.submitRegistrationDetailsFromExcel(sheetName, rowIndex);
	}

	@Then("User should get error message for register")
	public void user_should_get_eror_message_for_register() {
		String actualOutput = "";

		// Get the expected output 
		String expectedOutput = outputAndValidation[0];

		// Get the validation
		String validation = outputAndValidation[1];

		switch (validation.toLowerCase()) {
		case "username_empty":
			actualOutput = registerPage.errormsgEmptyUser();
			break;
		case "password_empty":
			actualOutput = registerPage.errormsgEmptyPw();
			break;
		case "confirmpassword_empty":
			actualOutput = registerPage.errormsgEmptyConfPw();
			break;
		case "username_invalid":
		case "password_invalid":
			actualOutput = registerPage.getErrormsg();
			break;
		}

		LogHelper.info("Comparing the actual message with the expected output: " + expectedOutput);
